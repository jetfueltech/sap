...
export const NewIntakePage: React.FC<NewIntakePageProps> = ({ onBack, onSubmit }) => {
  ...
  return (
    <div className="animate-fade-in pb-20 max-w-5xl mx-auto">
      <button onClick={onBack} className="mb-6 flex items-center text-slate-500 hover:text-slate-700 transition-colors font-medium">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden flex flex-col">
        ...
        <div className="bg-slate-50 px-8 py-6 border-b border-slate-100 flex justify-between items-center">
          <div>
            <h3 className="text-xl font-bold text-slate-800">New Client Intake Submission</h3>
            <p className="text-sm text-slate-500">Enter potential client details manually</p>
          </div>
          <div className="flex items-center space-x-3">
            <button onClick={fillMockData} className="text-xs text-blue-600 font-bold px-4 py-2 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors border border-blue-100">
              Auto-fill Mock Data
            </button>
          </div>
        </div>
        ...
        <div className="px-8 pt-8">
          <div className="flex items-center justify-between relative px-4">
            <div className="absolute left-0 top-4 w-full h-0.5 bg-slate-100 -z-10"></div>
            {steps.map((label, i) => (
              <div key={i} className="flex flex-col items-center cursor-pointer group bg-white px-2" onClick={() => setStep(i + 1)}>
                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold transition-all border-2 ${step > i ? 'bg-green-500 border-green-500 text-white' : step === i + 1 ? 'bg-blue-600 border-blue-600 text-white ring-4 ring-blue-50' : 'bg-white border-slate-200 text-slate-400 group-hover:border-slate-300'}`}>
                  {step > i ? '✓' : i + 1}
                </div>
                <span className={`text-xs mt-2 font-bold uppercase tracking-wider ${step === i + 1 ? 'text-blue-600' : 'text-slate-400'}`}>{label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="p-8">
          {step === 1 && (
            <div className="space-y-6 animate-fade-in max-w-3xl mx-auto">
              <h4 className="text-lg font-bold text-slate-800 border-b border-slate-100 pb-3">Client Demographics</h4>
          ...
          {step === 2 && (
            <div className="space-y-6 animate-fade-in max-w-3xl mx-auto">
              <h4 className="text-lg font-bold text-slate-800 border-b border-slate-100 pb-3">Incident & Vehicle</h4>
              <div className="grid grid-cols-2 gap-6">
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <textarea className={inputClass + " h-32 resize-none"} value={incidentInfo.description} onChange={e => setIncidentInfo({...incidentInfo, description: e.target.value})} placeholder="Describe how the accident happened..." />
                </div>
              </div>
              
              <div className="bg-slate-50 p-6 rounded-xl border border-slate-100 mt-4">
                <h5 className="font-bold text-slate-700 mb-4 text-sm uppercase tracking-wide">Client Vehicle Information</h5>
          ...
          {step === 3 && (
            <div className="space-y-8 animate-fade-in max-w-3xl mx-auto">
              {/* Defendant */}
              <div>
                <h4 className="text-lg font-bold text-slate-800 border-b border-slate-100 pb-3 mb-4 flex justify-between items-center">
                    <span>Defendant (At-Fault)</span>
                    <label className="flex items-center space-x-2 cursor-pointer bg-slate-50 px-3 py-1 rounded-lg border border-slate-200">
                        ...
                        <span className="text-xs font-bold text-slate-600 uppercase tracking-wide">Uninsured Driver?</span>
                    </label>
                </h4>
              ...
              {/* Client */}
              <div>
                <h4 className="text-lg font-bold text-slate-800 border-b border-slate-100 pb-3 mb-4">Client Insurance</h4>
                <div className="grid grid-cols-12 gap-6">
                  <div className="col-span-6">
               ...
               {/* Other */}
               <div>
                <h4 className="text-lg font-bold text-slate-800 border-b border-slate-100 pb-3 mb-4">Other Party Insurance</h4>
                <div className="grid grid-cols-12 gap-6">
          ...
          {step === 4 && (
             <div className="space-y-6 animate-fade-in max-w-3xl mx-auto">
               <h4 className="text-lg font-bold text-slate-800 border-b border-slate-100 pb-3">Medical Treatment</h4>
               <div>
               ...
               <div>
                 ...
                 <textarea className={inputClass + " h-40 resize-none"} placeholder="List names and locations of all medical providers seen..." value={medicalInfo.providers} onChange={e => setMedicalInfo({...medicalInfo, providers: e.target.value})} />
               </div>
             </div>
          )}
          ...
          {step === 5 && (
            <div className="space-y-6 animate-fade-in max-w-3xl mx-auto">
              <h4 className="text-lg font-bold text-slate-800 border-b border-slate-100 pb-3">Key Documents</h4>
              <p className="text-sm text-slate-600 mb-6 bg-blue-50 p-4 rounded-xl border border-blue-100 text-blue-800 flex items-start">
                  ...
                  Please simulate uploading the following key items required for valid intake processing. (Images & PDF supported)
              </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                 {[
                   ...
                   { id: 'photo', label: 'Scene/Injury Photos' }
                    ...
                    <div key={type.id} className="border border-dashed border-slate-300 bg-slate-50 rounded-xl p-6 hover:bg-white hover:border-blue-400 hover:shadow-md transition-all relative group text-center cursor-pointer">
                        ...
                        <div className="flex flex-col items-center space-y-3 pointer-events-none">
                            <div className="w-12 h-12 bg-white border border-slate-200 text-blue-600 rounded-full flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
              ...
              {documents.length > 0 && (
                <div className="mt-8">
                   <h5 className="text-xs font-bold uppercase text-slate-500 mb-4 tracking-wider">Attached Files ({documents.length})</h5>
                   <div className="space-y-3">
                     {documents.map((doc, i) => (
                       <div key={i} className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 text-sm hover:border-blue-300 transition-colors">
                          <div className="flex items-center space-x-4 overflow-hidden">
                             <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${doc.mimeType?.includes('pdf') ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
                             ...
                             <div className="truncate">
                               ...
                               <p className="text-xs text-slate-500 font-medium">{doc.type} • {doc.mimeType?.includes('pdf') ? 'PDF' : 'Image'}</p>
                             </div>
                          </div>
        ...
        <div className="bg-slate-50 px-8 py-5 border-t border-slate-100 flex justify-between flex-shrink-0">
           ...
           <button
             ...
             disabled={loading}
             className={`px-8 py-2.5 bg-indigo-600 text-white rounded-lg font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all flex items-center ${loading ? 'opacity-75 cursor-not-allowed' : ''}`}
           >
             {loading ? (
               <>
